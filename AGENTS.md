...
- i18n：`i18next` + `react-i18next`，翻译文件在 `frontend/src/i18n/{zh,en,vi}/`，命名空间 `common`/`dashboard`/`auth`/`errors`/`templates`
...
- CI 有 `test_i18n_consistency.py` 校验 key 漂移（覆盖 zh/en/vi 三语）
